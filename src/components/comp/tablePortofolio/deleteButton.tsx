...
export function DeleteButtonColumn({
  ...
  const deleteVendorMutation = useMutation({
    mutationFn: async (): Promise<TPorto | null[]> => {
      const response = await fetch(`${clientEnv.API_URL}/portfolio/${rowId}`, {
        method: 'DELETE',
      })
  ...
  const handleDelete: IHandleDelete = async (): Promise<void> => {
    toast.promise(deleteVendorMutation.mutateAsync(), {
      ...
      success: async () => {
        ...
        setShowDialog(false)
        return 'Data portofolio berhasil dihapus'
      },
      error: (error: Error) => {
        setShowDialog(false)
    ...
    mutationFn: async () => {
      const responses = await Promise.all(
        selectedIds.map((id: any) =>
          fetch(`${serverEnv.API_URL}/portfolio/${id}`, {
            method: 'DELETE',
          }).then((res) => {
            if (!res.ok) {
  ...
  const handleDelete = async () => {
    toast.promise(deleteVendorsMutation.mutateAsync(), {
      ...
      success: () => {
        ...
        return 'Data portfolio berhasil dihapus'
      },
      error: (error: Error) => {
        setShowDialog(false)
        setTimeout(onDeleteComplete, 100)
